...
getExtraInstances fp = do
  ...
  case parseRes of
         ...
         ParseFailed src err -> return . Left . InternalError
                                  $ "extra instance parse failed\n" ++ show src ++ ": " ++ show err
...
renderHsModuleForDotProto eis dp importCtxt =
    (header ++) . prettyPrint <$> hsModuleForDotProto eis dp importCtxt
  where
    header = unlines [ "{-# LANGUAGE DeriveGeneric #-}"
                     , "{-# LANGUAGE DataKinds #-}"
                     , "{-# LANGUAGE GADTs #-}"
                     , "{-# LANGUAGE OverloadedStrings #-}"
...
ctxtImports = fmap (map mkImportDecl . nub)
            . traverse (modulePathModName . dotProtoTypeInfoModulePath)
            . M.elems
  where
    mkImportDecl modName = importDecl_ modName True Nothing Nothing
...
qualifiedModuleName :: String -> HsQName
qualifiedModuleName qualifier modName = Qual (Module qualifier) (HsIdent modName)

haskellName, jsonpbName, grpcName, protobufName :: String -> HsQName
haskellName  = qualifiedModuleName "Hs"
jsonpbName   = qualifiedModuleName "HsJSONPB"
grpcName     = qualifiedModuleName "HsGRPC"
protobufName = qualifiedModuleName "HsProtobuf"
     ...
     let conDecl = recDecl_ (HsIdent serviceName)
         ...
         serviceClientD =
             let clientFuncD = match_ (HsIdent clientFuncName)
                 ...
                 clientRecE = foldl (`HsInfixApp` apOp)
